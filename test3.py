...
fields = ["Date", "Time", "Org", "Sub Org", "Name", "Cmds", "Description", "Upload Picture", "Area", "Type of Incident", "Case", "Longitude", "Latitude"]
entries = {}
...
for idx, field in enumerate(fields):
    ...
    if field == "Type of Incident":
        ...
        incident_combobox = ttk.Combobox(data_entry_frame, textvariable=incident_var, values=["Fire Raid", "Tat Killing", "SB Attk", "IDE Attk", "Ambush", "Polio Incident", "Abduction", "Extortion", "Robberies"], font=("Arial", 10), width=15)
...
buttons_frame.grid(row=2, column=0, columnspan=8, pady=10, padx=20, sticky="w")
...
pictures_frame.grid(row=3, column=0, padx=50, pady=25, sticky="n")
...
# Activity Logs
activity_logs_frame = tk.LabelFrame(main_frame, text="Activity Logs")
activity_logs_frame.grid(row=4, column=0, columnspan=8, fill=tk.X, padx=20, pady=20)

columns = ("S#", "Date", "Time", "Org", "Sub Org", "Name", "Comds", "Watch List", "Brief/Description", "Picture", "Area", "Type of Incident", "Cas", "Long/Lati", "Google Earth")
activity_tree = ttk.Treeview(activity_logs_frame, columns=columns, show='headings')

for col in columns:
    activity_tree.heading(col, text=col)
    activity_tree.column(col, minwidth=0, width=80)
